...
if not os.path.exists(f'{filepath}\{CollectionName}'):
    os.mkdir(f'{filepath}\{CollectionName}')

if not os.path.exists(f'{filepath}\{CollectionName}\image_data'):
    os.mkdir(f'{filepath}\{CollectionName}\image_data')
...
for i in range(iter):
    ...
    if "assets" in data:
        for asset in data["assets"]:
          ...
          if os.path.exists(f'{filepath}\{CollectionName}\image_data\{formatted_number}.json'):
              ...
              stats["AlreadyDownloadedData"] += 1
                ...
                dfile = open(f"{filepath}\{CollectionName}\image_data\{formatted_number}.json", "w+")
          ...
          if os.path.exists(f'{filepath}\{CollectionName}\{formatted_number}.png'):
          ...
          # If the URL returns status code "200 Successful", save the image into the "images" folder.
            if image.status_code == 200:
                file = open(f"{filepath}\{CollectionName}\{formatted_number}.png", "wb+")
